...
main = do
  ...
  let win_config = ((100, 80), (window_width, window_height), window_title)
      ...
      puck = objectGroup "puckGroup" []                                         
      
      game_score = Score 0 0                                                    -- initialize game score to 0 for both players
      input_map = []                                                            -- define key mapping to functions
  funInit win_config game_map [strikers, puck] () game_score input_map gameCycle (Timer fps_delay) img_list -- initialize game with defined properties above
